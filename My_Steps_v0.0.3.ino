...
#define sonar1limit 5       //см, если обнаружена дистанция меньше, чем это число, то сонар считается сработавшим
#define sonar2limit 5       //см, если обнаружена дистанция меньше, чем это число, то сонар считается сработавшим

#define sensorInactiveTime 2500 //мс, время после срабатывания сонара, в течение которого сенсор игнорит следующие срабатывания
#define stairsCount 7        //количество ступенек
...
#define Speed 1.6                                                                    //скорость включения 
...
void setup(){
    ...
    delay(500); 
    for (byte i = 0; i < stairsCount; i++)  StairsArrayPWM[i] = 0;              //задаём 0 уровень яркости ступеням от 2 до stairsCount-1
    PWM_Output();
    Ainimation_First();
    StairsArrayPWM[0] =  initialPWMvalue;          //Дежурный уровень яркости 1 и последеней ступени
    StairsArrayPWM[stairsCount-1] = initialPWMvalue;
    PWM_Output();
...
void loop(){
  ...
  else if (sensor1trig && AllStairs_ON) {
  AllLight_OffDelay = waitForTurnOff ;           // если освещение уже полностью включено, обновить отсчет задержки выключения освещения
  direction = true;
  }     
    ...
    ON_TopBottom = true;                                                           // прекратить включение освещения сверху вниз
  ...
  else if (sensor2trig && AllStairs_ON) {
  AllLight_OffDelay = waitForTurnOff ;         // если освещение уже полностью включено, обновить отсчет задержки выключения освещения 
  direction = false;
  }    
    ...
    ON_BottomTop = true;                                                           // прекратить включение освещения снизу вверх
...
void PWM_Output() {  
  for (int i = 0; i < stairsCount; i++) Tlc.set(i, StairsArrayPWM[i]);                       // яркость ступеней в диапазоне 0...4096 
  Tlc.update();
}
...
void check_sensors(){
  ...
  void Ainimation_First(){
    ...
    void Delay_Read_Sensors(){
       delay(30);                                                                      // задержка для обеспечения паузы перед следующим опросом сонаров
